...
const JobApplicationFormWithAPI = ({ jobId }) => {
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      try {
        // Try to use the API for resume parsing
        analysisResult = await resumeParser.parseResume(file);
      } catch (parseError) {
        console.error('API resume parsing failed, using fallback:', parseError);
        ...
        analysisResult = resumeParser.fallbackParseResume(fileText);
      }
      
      // Try to match with job requirements
      try {
        const matchResult = await resumeParser.matchResumeWithJob(analysisResult, jobId);
        analysisResult = { ...analysisResult, ...matchResult };
      } catch (matchError) {
        console.error('Job matching failed:', matchError);
        // Continue without job matching data
      }
      ...
      const formattedAnalysis = {
        match_score: analysisResult.match_score || 75, // Default if not available
        skills: {
          matched: analysisResult.skills?.matched || analysisResult.skills || [],
          missing: analysisResult.skills?.missing || [],
        },
        experience_years: analysisResult.experience?.years || 
                         (analysisResult.experience?.relevant_years || 
                          analysisResult.experience?.total_years || 0),
        education: analysisResult.education?.highest_degree || 
                  (analysisResult.education?.degrees?.[0] || 'Not specified'),
        recommendations: analysisResult.recommendations || [
          'Make sure your resume highlights relevant skills for this position',
          'Consider adding more details about your work experience',
        ],
      };
      
      setResumeAnalysis(formattedAnalysis);
      ...
      toast({
        ...
        description: 'We were unable to analyze your resume. You can still submit your application.',
        status: 'warning',
        duration: 5000,
        isClosable: true,
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await submitApplication(jobId, applicationData, formData.useProfileResume ? null : resumeFile);
            ...
            >
              {!resumeFile ? (
                ...
                <VStack spacing={3}>
                  {isUploading ? (
                    <>
                      <Text fontWeight="medium">Uploading and analyzing resume...</Text>
                      <Progress
...
export default JobApplicationFormWithAPI;
